        ...
        <div id="panel-jira" class="main-panel">
            ...
            <section class="charts-row-two">
                <div class="card chart-card">
                    ...
                    <div class="chart-container-pie" style="height: 380px;">
    ...
    <script>
        ...
        function renderJiraDashboard() {
            ...
            if (!jiraChartsInitialized) {
                ...
                if (jiraCategoriesChart) {
                    jiraCategoriesChart.data.labels = data.months;
                    data.monthly_categories.forEach((c, idx) => {
                        if (jiraCategoriesChart.data.datasets[idx]) {
                            jiraCategoriesChart.data.datasets[idx].data = c.values;
                        }
                    });
                ...
                // Update Hours Chart
                if (jiraHoursChart) {
                    jiraHoursChart.data.labels = data.months;
                    data.monthly_hours.forEach((h, idx) => {
                        if (jiraHoursChart.data.datasets[idx]) {
                            jiraHoursChart.data.datasets[idx].data = h.values;
                        }
                    });
        ...
        function renderJiraCategoriesChart(months, monthlyCategories) {
            ...
            jiraCategoriesChart = new Chart(ctx, {
                ...
                data: {
                    labels: months,
                    datasets: monthlyCategories.map((c, idx) => ({
                        label: c.name,
                        data: c.values,
                        backgroundColor: chartColors[idx % chartColors.length],
                        borderColor: colors.cardBorder,
        ...
        function renderJiraHoursChart(months, monthlyHours) {
            ...
            jiraHoursChart = new Chart(ctx, {
                ...
                data: {
                    labels: months,
                    datasets: monthlyHours.map((h, idx) => ({
                        label: h.name,
                        data: h.values,
                        backgroundColor: chartColors[(idx + 5) % chartColors.length],
                        borderColor: colors.cardBorder,
                        borderWidth: 1
                    }))
                },
        ...
        function updateJiraChartsTheme() {
            ...
            if (jiraCategoriesChart) {
                jiraCategoriesChart.options.scales.x.ticks.color = colors.textSecondary;
            ...
            if (jiraHoursChart) {
                jiraHoursChart.options.scales.x.ticks.color = colors.textSecondary;
