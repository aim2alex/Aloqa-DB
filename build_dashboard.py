    ...
    try:
        ...
        def get_kassa_period_data(df_period, months_to_include):
            ...
            for m in months_to_include:
                ...
                # Blocks counts (USD + EUR + RUB + UZS)
                def get_row_blocks_sum(df_slice):
                    s = (
                        df_slice['COLLECTION_DETAIL_COUNT_USD'].fillna(0) +
                        df_slice['COLLECTION_DETAIL_COUNT_EUR'].fillna(0) +
                        df_slice['COLLECTION_DETAIL_COUNT_RUB'].fillna(0) +
                        df_slice['COLLECTION_DETAIL_COUNT_UZS'].fillna(0)
                    ).sum()
                    return int(s)

                t1_qr_counts.append(get_row_blocks_sum(df_m[df_m['ORDDSC_NAME'] == type1]))
                t2_qr_counts.append(get_row_blocks_sum(df_m[df_m['ORDDSC_NAME'] == type2]))
            ...
            return {
                ...
                'type1_qr_data': t1_qr_counts,
                'type2_qr_data': t2_qr_counts
            }
        ...
        kassa_stats = {
            'all': {'bags': 0, 'sent_bags': 0, 'blocks': 0, 'sum_uzs': 0.0, 'sum_usd': 0.0, 'sum_eur': 0.0, 'sum_rub': 0.0, 'months': [], 'type1_data': [], 'type2_data': [], 'type1_qr_data': [], 'type2_qr_data': []},
            'q1': {'bags': 0, 'sent_bags': 0, 'blocks': 0, 'sum_uzs': 0.0, 'sum_usd': 0.0, 'sum_eur': 0.0, 'sum_rub': 0.0, 'months': [], 'type1_data': [], 'type2_data': [], 'type1_qr_data': [], 'type2_qr_data': []},
            'q2': {'bags': 0, 'sent_bags': 0, 'blocks': 0, 'sum_uzs': 0.0, 'sum_usd': 0.0, 'sum_eur': 0.0, 'sum_rub': 0.0, 'months': [], 'type1_data': [], 'type2_data': [], 'type1_qr_data': [], 'type2_qr_data': []}
        }
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .card-kassa-qr::before { background: linear-gradient(90deg, #10b981, #06b6d4); }
...
<body>
    ...
    <div class="container">
            ...
            <section class="charts-row-two-custom">
                <div class="card chart-card span-2" style="min-height: 600px;">
                    ...
                    <div class="chart-container-pie" style="position: relative; display: flex; flex-direction: column; align-items: center; justify-content: flex-start; min-height: 500px; width: 100%; padding: 10px 0;">
                        ...
                        <div id="doughnut-products-legend" style="display: flex; flex-wrap: wrap; justify-content: center; gap: 12px 20px; width: 100%; max-height: 220px; overflow-y: auto; padding: 5px 10px;">
        ...
        <div id="panel-cards" class="main-panel" style="display: none;">
            ...
            <section class="kpi-grid">
                ...
                <div class="card card-loans-yellow">
                    <div class="card-header">
                        <span class="card-title">Выпущено карт</span>
                    ...
                    <div class="card-body">
                        <span class="card-value" id="kpi-cards-issued">0</span>
                        <div class="card-meta">
                            <span>Всего выпущенных карт за период</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="card card-mc-total">
                    <div class="card-header">
                        <span class="card-title">Средний остаток на счетах</span>
                        <div class="card-icon" style="background: rgba(59, 130, 246, 0.15); color: #3b82f6;">
                            <svg class="svg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <line x1="12" y1="1" x2="12" y2="23"></line>
                                <path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"></path>
                    ...
                    <div class="card-body">
                        <span class="card-value" id="kpi-cards-balance">0</span>
                        <div class="card-meta">
                            <span>Среднее значение остатков на картах</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="card card-mc-completed">
                    <div class="card-header">
                        <span class="card-title">Кредитный оборот</span>
                        <div class="card-icon" style="background: rgba(16, 185, 129, 0.15); color: #10b981;">
                            <svg class="svg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    <div class="card-body">
                        <span class="card-value" id="kpi-cards-credit">0</span>
                        <div class="card-meta">
                            <span>Сумма поступлений за период</span>
                        </div>
                    </div>
                </div>
            </section>
        ...
        <div id="panel-scoring" class="main-panel" style="display: none;">
            ...
            <!-- Row 1: Aggregated metrics cards -->
            <section class="kpi-grid" id="scoring-top-kpis">
                <!-- Total Applications Scored -->
                <div class="card card-mc-total">
                    <div class="card-header">
                        <span class="card-title">Всего отскорено заявок</span>
                        <div class="card-icon" style="background: rgba(59, 130, 246, 0.15); color: #3b82f6;">
                            <svg class="svg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path>
                                <polyline points="14 2 14 8 20 8"></polyline>
                    ...
                    <div class="card-body">
                        <span class="card-value" id="kpi-scoring-total-value">0</span>
                        <div class="card-meta">
                            <span>Зарегистрированные заявки</span>
                        </div>
                    </div>
                ...
                <!-- Average Scored per Month -->
                <div class="card card-mc-completed">
                    <div class="card-header">
                        <span class="card-title">В среднем за месяц</span>
                        <div class="card-icon" style="background: rgba(16, 185, 129, 0.15); color: #10b981;">
                            <svg class="svg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <circle cx="12" cy="12" r="10"></circle>
                                <polyline points="12 6 12 12 16 14"></polyline>
                    ...
                    <div class="card-body">
                        <span class="card-value" id="kpi-scoring-average-value">0</span>
                        <div class="card-meta">
                            <span>Среднемесячный показатель</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="card card-loans-yellow">
                    <div class="card-header">
                        <span class="card-title">Общее кол-во моделей</span>
                        <div class="card-icon" style="background: rgba(245, 158, 11, 0.15); color: #f59e0b;">
                            <svg class="svg-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    <div class="card-body">
                        <span class="card-value" id="kpi-scoring-models-value">0</span>
                        <div class="card-meta">
                            <span>Активные скоринговые модели</span>
                        </div>
                    </div>
                </div>
            </section>
        ...
        <div id="panel-kassa" class="main-panel" style="display: none;">
            ...
            <!-- Row 3: Vertical Grouped Bar Charts -->
            <section class="charts-row" style="margin-top: 2rem; display: grid; grid-template-columns: repeat(auto-fit, minmax(450px, 1fr)); gap: 1.5rem;">
                <!-- Chart 1: Bags sent -->
                <div class="card chart-card" style="padding: 1.5rem 2rem;">
                    <div class="chart-card-header" style="margin-bottom: 1rem;">
                ...
                <!-- Chart 2: Blocks QR collected -->
                <div class="card chart-card" style="padding: 1.5rem 2rem;">
                    <div class="chart-card-header" style="margin-bottom: 1rem;">
                        <span class="chart-card-title">Динамика сбора блоков с QR по направлениям</span>
                    </div>
                    <div class="chart-container-bar" style="height: 380px; position: relative; margin-top: 1rem;">
                        <canvas id="chart-kassa-qr-blocks"></canvas>
                    </div>
                </div>
            </section>
        ...
        <div id="panel-jira" class="main-panel">
            ...
            <section style="display: flex; flex-direction: column; gap: 1rem;">
                <div class="quarters-grid" style="grid-template-columns: 1fr 1fr;">
                    ...
                    <!-- Right Card: Выполнено задач по категориям -->
                    <div class="card" style="padding: 1.5rem 2rem; position: relative;">
                        <!-- Header -->
                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0.5rem;">
                            <div>
                                <div style="font-family: 'Outfit', sans-serif; font-size: 1.3rem; font-weight: 700; color: var(--text-primary);">Выполнено</div>
                                <div style="font-size: 0.8rem; color: var(--text-secondary); margin-top: 2px;">Кол-во задач по категориям, шт</div>
                            </div>
                        </div>
                        
                        <!-- Legend -->
                        <div style="display: flex; gap: 15px; margin-top: 5px; margin-bottom: 5px; font-size: 0.8rem;">
                            <div style="display: flex; align-items: center; gap: 6px;">
                                <span style="display: inline-block; width: 8px; height: 8px; border-radius: 50%; background-color: #06b6d4;"></span>
                                <span style="color: var(--text-secondary);">Ранее</span>
                            </div>
                            <div style="display: flex; align-items: center; gap: 6px;">
                                <span style="display: inline-block; width: 8px; height: 8px; border-radius: 50%; background-color: #eab308;"></span>
                                <span style="color: var(--text-secondary);">Прошлый месяц</span>
                            </div>
                            <div style="display: flex; align-items: center; gap: 6px;">
                                <span style="display: inline-block; width: 8px; height: 8px; border-radius: 50%; background-color: #10b981;"></span>
                                <span style="color: var(--text-secondary);">Текущий месяц</span>
                            </div>
                        </div>
    ...
    <script>
        ...
        function switchMainTab(tabName) {
            ...
            // Ensure scroll is reset immediately after rendering settles and layout calculation updates
            resetScroll();
        ...
        function renderScoringDashboard() {
            ...
            animateValue('kpi-scoring-average-value', 0, avgPerMonth, 800);
            animateValue('kpi-scoring-models-value', 0, totalModels, 800);
        ...
        function renderKassaDashboard() {
            ...
            // Render vertical grouped charts
            const themeColors = getThemeColors();

            // 1. Bags sent chart
            const ctxBags = document.getElementById('chart-kassa-sent-bags').getContext('2d');
            if (kassaSentBagsChart) {
                kassaSentBagsChart.destroy();
            }

            kassaSentBagsChart = new Chart(ctxBags, {
                type: 'bar',
                data: {
                    labels: data.months,
                    datasets: [
            ...
            // 2. QR Blocks collected chart
            const ctxQR = document.getElementById('chart-kassa-qr-blocks').getContext('2d');
            if (kassaQrBlocksChart) {
                kassaQrBlocksChart.destroy();
            }

            kassaQrBlocksChart = new Chart(ctxQR, {
                type: 'bar',
                data: {
                    labels: data.months,
                    datasets: [
        ...
        function updateKassaChartsTheme() {
            ...
            if (kassaQrBlocksChart) {
                kassaQrBlocksChart.options.scales.x.ticks.color = colors.textSecondary;
                kassaQrBlocksChart.options.scales.y.ticks.color = colors.textSecondary;
                kassaQrBlocksChart.options.scales.y.grid.color = colors.gridColor;
                kassaQrBlocksChart.options.plugins.legend.labels.color = colors.textSecondary;
                kassaQrBlocksChart.update();
            }
        }
        ...
        function renderLoansProductsChart(productsData) {
            ...
            if (legendContainer) {
                legendContainer.innerHTML = productsData.map((item, idx) => {
                    ...
                    return `
                    <div style="display: flex; align-items: center; gap: 6px; cursor: pointer; transition: opacity 0.2s; font-size: 16px;" onclick="toggleChartSlice('loansProductsChart', ${idx}, this)" title="${item.name}">
                ...
                options: {
                    ...
                    scales: {
                        ...
                        y: {
                            ...
                            ticks: {
                                color: colors.textSecondary,
                                font: { size: 15, family: 'Inter', weight: '500' }
                            }
                        }
                    },
                    plugins: {
        ...
        function renderJiraDashboard() {
            ...
            const jiraTimeSpentData = {
                ...
                all: {
                    total: 1626.7,
                    categories: [
                        { name: "Поддержка и инциденты", value: 13.75, color: "#ef4444" },
                        { name: "Внутренняя автоматизация", value: 30.70, color: "#8b5cf6" },
                        { name: "Требования регулятора", value: 16.30, color: "#06b6d4" },
                        { name: "Новые продукты для клиентов", value: 38.24, color: "#10b981" },
                        { name: "KPI-цели (KPI план бизнеса)", value: 1.01, color: "#f97316" },
                        { name: "Без категории", value: 0.01, color: "#64748b" }
                    ]
                }
            };
            ...
            // Update Jira Categories Stacked Chart
            const jiraCategoriesStackedData = {
                q1: {
                    labels: [
                        "Требования регулятора",
                        "Новые продукты для клиентов",
                        "KPI-цели (KPI план бизнеса)",
                        "Поддержка и инциденты",
                        "Внутренняя автоматизация",
                        "Без категории"
                    ],
                    datasets: [
                        { label: "Ранее", data: [69, 114, 4, 227, 101, 2], backgroundColor: "#06b6d4" },
                        { label: "Прошлый месяц", data: [0, 0, 0, 0, 0, 0], backgroundColor: "#eab308" },
                        { label: "Текущий месяц", data: [0, 0, 0, 0, 0, 0], backgroundColor: "#10b981" }
                    ]
                },
                q2: {
                    labels: [
                    ...
                    datasets: [
                        { label: "Ранее", data: [33, 54, 1, 141, 68, 1], backgroundColor: "#06b6d4" },
                        { label: "Прошлый месяц", data: [10, 26, 0, 48, 54, 0], backgroundColor: "#eab308" },
                        { label: "Текущий месяц", data: [0, 0, 0, 0, 0, 0], backgroundColor: "#10b981" }
                    ]
                },
                all: {
                    ...
                    datasets: [
                        { label: "Ранее", data: [97, 158, 5, 362, 160, 2], backgroundColor: "#06b6d4" },
                        { label: "Прошлый месяц", data: [10, 26, 0, 48, 54, 0], backgroundColor: "#eab308" },
                        { label: "Текущий месяц", data: [0, 0, 0, 0, 0, 0], backgroundColor: "#10b981" }
                    ]
                }
            ...
            const stackedCanvas = document.getElementById('chart-jira-categories-stacked');
            if (stackedCanvas) {
                const stackedCtx = stackedCanvas.getContext('2d');
                const axisColors = getThemeColors();
                
                if (jiraCategoriesStackedChart) {
                    jiraCategoriesStackedChart.destroy();
                }
        ...
        const jiraBusinessDirectionsData = {
            all: [
                { name: "Camunda (ФЛ)", acceptance: 7, in_progress: 33, completed: 253, total: 293 },
                { name: "Кредиты БЭК (ФЛ и ЮЛ)", acceptance: 13, in_progress: 24, completed: 223, total: 260 },
                { name: "КФО ФЛ", acceptance: 7, in_progress: 9, completed: 68, total: 84 },
                { name: "Карточный бизнес", acceptance: 8, in_progress: 13, completed: 50, total: 71 },
                { name: "IT", acceptance: 0, in_progress: 2, completed: 32, total: 34 },
                { name: "Колл-центр", acceptance: 0, in_progress: 5, completed: 29, total: 34 },
                { name: "Бухгалтерия и МСФО", acceptance: 8, in_progress: 4, completed: 20, total: 32 },
                { name: "AML", acceptance: 4, in_progress: 5, completed: 21, total: 30 },
                { name: "Проблемные активы", acceptance: 1, in_progress: 2, completed: 26, total: 29 },
                { name: "Управление кадрами", acceptance: 0, in_progress: 4, completed: 25, total: 29 },
                { name: "КФО ФЛ", acceptance: 0, in_progress: 0, completed: 26, total: 26 },
                { name: "Риск менеджмент и расчет резервов", acceptance: 2, in_progress: 3, completed: 19, total: 24 },
                { name: "Электронная коммерция", acceptance: 5, in_progress: 4, completed: 11, total: 20 },
                { name: "Отчетность", acceptance: 2, in_progress: 7, completed: 10, total: 19 },
                { name: "Кассовые операции", acceptance: 2, in_progress: 9, completed: 7, total: 18 },
                { name: "Залоги", acceptance: 1, in_progress: 2, completed: 14, total: 17 },
                { name: "Валютные операции", acceptance: 2, in_progress: 2, completed: 12, total: 16 },
                { name: "Корпоративные услуги", acceptance: 3, in_progress: 1, completed: 7, total: 11 },
                { name: "Розничные услуги", acceptance: 0, in_progress: 1, completed: 10, total: 11 },
                { name: "AUDIT", acceptance: 1, in_progress: 2, completed: 5, total: 8 },
                { name: "Клиенты и контрагенты", acceptance: 0, in_progress: 3, completed: 4, total: 7 },
                { name: "Аккредитивы Гарантии (бэк-офис)", acceptance: 0, in_progress: 0, completed: 6, total: 6 },
                { name: "Бух. Главная книга и клиринг (БЭК)", acceptance: 0, in_progress: 1, completed: 5, total: 6 },
                { name: "Депозиты ЮЛ", acceptance: 1, in_progress: 1, completed: 4, total: 6 },
                { name: "КФО ЮЛ", acceptance: 0, in_progress: 1, completed: 5, total: 6 },
                { name: "Методология", acceptance: 0, in_progress: 2, completed: 4, total: 6 },
                { name: "Факторинг", acceptance: 1, in_progress: 0, completed: 5, total: 6 },
                { name: "Депозиты ФЛ", acceptance: 1, in_progress: 1, completed: 3, total: 5 },
                { name: "Депозиты ФЛ и ЮЛ", acceptance: 0, in_progress: 1, completed: 4, total: 5 },
                { name: "Заработная плата", acceptance: 0, in_progress: 0, completed: 5, total: 5 },
                { name: "Антифрод/Кибербезопасность", acceptance: 1, in_progress: 0, completed: 2, total: 3 },
                { name: "undefined", acceptance: 0, in_progress: 1, completed: 1, total: 2 },
                { name: "Автопогашение", acceptance: 0, in_progress: 1, completed: 1, total: 2 },
                { name: "Автосписание", acceptance: 0, in_progress: 0, completed: 2, total: 2 },
                ...
                { name: "Документарные опер. (бэк-офис)", acceptance: 1, in_progress: 0, completed: 0, total: 1 },
                { name: "Казначейство", acceptance: 0, in_progress: 0, completed: 1, total: 1 },
                { name: "Объекты", acceptance: 0, in_progress: 1, completed: 0, total: 1 }
            ],
            q1: [
                { name: "Camunda (ФЛ)", acceptance: 3, in_progress: 18, completed: 150, total: 171 },
                { name: "Кредиты БЭК (ФЛ и ЮЛ)", acceptance: 7, in_progress: 15, completed: 118, total: 140 },
                { name: "КФО ФЛ", acceptance: 1, in_progress: 5, completed: 46, total: 52 },
                { name: "Карточный бизнес", acceptance: 3, in_progress: 4, completed: 31, total: 38 },
                { name: "Проблемные активы", acceptance: 0, in_progress: 2, completed: 21, total: 23 },
                { name: "КФО ФЛ", acceptance: 0, in_progress: 0, completed: 20, total: 20 },
                { name: "AML", acceptance: 1, in_progress: 4, completed: 14, total: 19 },
                { name: "Бухгалтерия и МСФО", acceptance: 4, in_progress: 2, completed: 12, total: 18 },
                { name: "Колл-центр", acceptance: 0, in_progress: 1, completed: 17, total: 18 },
                { name: "Управление кадрами", acceptance: 0, in_progress: 3, completed: 12, total: 15 },
                { name: "Кассовые операции", acceptance: 2, in_progress: 8, completed: 4, total: 14 },
                { name: "Валютные операции", acceptance: 2, in_progress: 1, completed: 8, total: 11 },
                { name: "Корпоративные услуги", acceptance: 3, in_progress: 1, completed: 5, total: 9 },
                { name: "Отчетность", acceptance: 1, in_progress: 1, completed: 7, total: 9 },
                { name: "Электронная коммерция", acceptance: 3, in_progress: 1, completed: 5, total: 9 },
                { name: "Риск менеджмент и расчет резервов", acceptance: 1, in_progress: 3, completed: 4, total: 8 },
                { name: "Розничные услуги", acceptance: 0, in_progress: 1, completed: 6, total: 7 },
                { name: "AUDIT", acceptance: 0, in_progress: 1, completed: 4, total: 5 },
                { name: "Залоги", acceptance: 0, in_progress: 0, completed: 5, total: 5 },
                { name: "Депозиты ФЛ", acceptance: 0, in_progress: 1, completed: 3, total: 4 },
                { name: "Депозиты ЮЛ", acceptance: 1, in_progress: 1, completed: 2, total: 4 },
                { name: "Клиенты и контрагенты", acceptance: 0, in_progress: 2, completed: 2, total: 4 },
                { name: "КФО ЮЛ", acceptance: 0, in_progress: 1, completed: 3, total: 4 },
                { name: "Факторинг", acceptance: 1, in_progress: 0, completed: 3, total: 4 },
                { name: "Заработная плата", acceptance: 0, in_progress: 0, completed: 3, total: 3 },
                { name: "Методология", acceptance: 0, in_progress: 2, completed: 1, total: 3 },
                { name: "undefined", acceptance: 0, in_progress: 1, completed: 1, total: 2 },
                { name: "Автопогашение", acceptance: 0, in_progress: 1, completed: 1, total: 2 },
                { name: "Аккредитивы Гарантии (бэк-офис)", acceptance: 0, in_progress: 0, completed: 2, total: 2 },
                { name: "Антифрод/Кибербезопасность", acceptance: 0, in_progress: 0, completed: 2, total: 2 },
                { name: "Автосписание", acceptance: 0, in_progress: 0, completed: 1, total: 1 },
                { name: "Бух. Главная книга и клиринг (БЭК)", acceptance: 0, in_progress: 0, completed: 1, total: 1 },
        ...
        function updateJiraChartsTheme() {
            ...
            if (jiraCategoriesStackedChart && jiraCategoriesStackedChart.options && jiraCategoriesStackedChart.options.scales) {
                if (jiraCategoriesStackedChart.options.scales.x) {
                    if (jiraCategoriesStackedChart.options.scales.x.grid) {
                        jiraCategoriesStackedChart.options.scales.x.grid.color = colors.gridColor;
                    }
                    if (jiraCategoriesStackedChart.options.scales.x.ticks) {
                        jiraCategoriesStackedChart.options.scales.x.ticks.color = colors.textSecondary;
                    }
                }
                if (jiraCategoriesStackedChart.options.scales.y && jiraCategoriesStackedChart.options.scales.y.ticks) {
                    jiraCategoriesStackedChart.options.scales.y.ticks.color = colors.textSecondary;
                }
                jiraCategoriesStackedChart.update();
            }
        }
